...
interface CardProps {
    ...
    title: string;
    link: string;
    type: "twitter" | "youtube" | "article"; // Added article type
    tags?: Tag[];
    createdAt: string;
...
export function Card({
    ...
    return (
        <div className="group bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow w-full max-w-[280px]">
            {/* Card Header */}
            <div className="p-3">
                <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2 min-w-0">
                        {type === "twitter" ? (
                            ...
                            <svg className="w-6 h-6 text-gray-600 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                                <path
                    ...
                    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button className="p-1 rounded-lg hover:bg-gray-50 transition-colors" aria-label="Share">
                            <ShareIcon className="w-3.5 h-3.5 text-gray-600" />
                        </button>
                        <button onClick={() => deleteContent(id)} className="p-1 rounded-lg hover:bg-gray-50 transition-colors" aria-label="Delete">
