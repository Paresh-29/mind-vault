...
export const ShareBrainModal: React.FC<ShareBrainModalProps> = ({ isOpen, onClose, itemCount = 0 }) => {
  ...
  const handleToggleShare = async () => {
    ...
    try {
      ...
      setError(error?.response?.data?.message || `Failed to ${isSharing ? "disable" : "enable"} sharing`);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md bg-white rounded-xl shadow-xl p-6 space-y-4">
        ...
        <div className="space-y-3">
          <button
            className={`w-full py-2 px-4 rounded-lg flex items-center justify-center gap-2 ${isSharing
